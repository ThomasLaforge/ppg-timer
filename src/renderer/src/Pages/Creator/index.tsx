...
import { api, trainingsAPI } from "../../App";
...
export default function Creator(){
    ...
    const match = useMatch("/creator/:trainingIndex");
    // @ts-ignore
    const params: CreateRouteParams = match?.params 
    // @ts-ignore
    const trainingIndex: number = params?.trainingIndex
    ...
    const completeTraining = async (redirectionPath: string) => {
        if(updateMode && JSON.stringify(trainingsDB.get(trainingIndex).data) === JSON.stringify(state)) {
            ...
            if(updateMode){
                trainingsDB.update(trainingIndex, state)
            }
            else {
                trainingsDB.create(state)
            }
    
            const trainingHash = MD5(JSON.stringify(state)).toString()
            try{
                const queryTraining = query(trainingsAPI, where("hash", "==", trainingHash));
                const apiTrainingsWithHash = await getDocs(queryTraining)
                if(apiTrainingsWithHash.docs.length < 1){
                    console.log('complete training 5');

                    await setDoc(doc(api, 'trainings', trainingHash), {
                        hash: trainingHash,
